...
| `ce-id` | string | ID события из оригинального CloudEvent; если не передан, SDK cloudevents 2.0 генерирует UUID v4 (заголовок присутствует всегда) |
| `ce-time` | string | Время события ISO 8601; если не передано, SDK подставляет текущее время (заголовок присутствует всегда) |
...
| `ce-bookingid` | string | ID бронирования (если доступен) |
...
```
> Входящий запрос содержит `users[{role: organizer}, {role: client}]` + `booking_uid`. Контроллер трансформирует в структуру `user`/`client`, `booking_uid` выносится в `ce-bookingid`.
...
```
> `booking_uid` удаляется из `event_data.metadata` до публикации и выносится в `ce-bookingid`.
...
Структура аналогична остальным GetStream событиям. `channel_id` используется как `booking_id` (`ce-bookingid`).
...
- `ce-bookingid`: значение `room` из JWT claims
